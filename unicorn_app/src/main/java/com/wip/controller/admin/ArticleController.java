...
import com.wip.dto.cond.ContentVO;
...
public class ArticleController extends BaseController {
    ...
    @ResponseBody
    public Object index2(@RequestBody ContentVO contentVO) {
        ContentCond contentCond = new ContentCond();
        contentCond.setCategory(contentVO.getCategory());
        contentCond.setTitle(contentVO.getTitle());

        PageInfo<ContentDomain> articles = contentService.getArticlesByCond(contentCond, contentVO.getPage(), contentVO.getLimit());
    ...
    @ApiOperation("文章内容页json")
    @GetMapping(value = "/json/{cid}")
    @ResponseBody
    public ContentDomain detail(@ApiParam(name = "cid", value = "文章主键", required = true)
    ...
    public APIResponse publishArticle(
        ...
        // 对象转化为json 保存到 algolia
        AsyncAPIClient client =
                new AsyncHttpAPIClientBuilder("CV1JXUSE00", "13f340aacbbcaf5e1940e1521b5baac2").build();

        AsyncIndex<ContentDomain> index = client.initIndex("yichengIndex", ContentDomain.class);
        index.addObject(contentDomain);
